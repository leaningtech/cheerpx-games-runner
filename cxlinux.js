...
async function handleMessage(m)
{
	...
	if(data.type == "port")
	{
		...
		cx = await CheerpXApp.create({devices:[{type:"bytes",url:"https://127.0.0.1:8083/debian_run_games.ext2",name:"block1"}],mounts:[{type:"ext2",dev:"block1",path:"/"},{type:"cheerpOS",dev:"/files",path:"/files"},{type:"devs",dev:"",path:"/dev"}]});
	...
	else if(data.type == "install")
	{
		...
		var ret = await cx.run("/bin/rm", ["-rf", `/files/${gameId}/`, "files/installer.exe", "/tmp/FDCONFIG.SYS"]);
		if(ret != 0)
			debugger;
		// TODO: Build autoexec
